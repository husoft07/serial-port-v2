...

        {
            ...
            if (File.Exists(dllPath))
            {
                try
                {
                    ...
                    {
                        // Menü oluşturmak için MyLibrary'deki CreateSampleMenu metodunu çalıştırıyoruz
                        var method = menuHelperType.GetMethod("CreateSampleMenu");
                        ToolStripMenuItem samplesMenuItem = new ToolStripMenuItem("TT Temel Erişim Komutları");
                        method.Invoke(null, new object[] { samplesMenuItem });
                        menuStrip1.Items.Add(samplesMenuItem);
                    }
                }
                catch (Exception ex)
                {
